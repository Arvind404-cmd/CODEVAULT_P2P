...
app.get('/api/ipfs/pins', async (req, res) => {
  if (!ipfsAvailable) {
    const pins = Array.from(localFileStore.keys()).map(cid => ({ cid, type: 'recursive' }));
...
app.post('/api/ipfs/connect', async (req, res) => {
  ...
  if (!ipfsAvailable) {
    return res.json({ success: true, message: 'Demo mode - peer connection simulated', demoMode: true });
  }
...
app.get('/api/benchmark/p2p/:cid', async (req, res) => {
  if (!ipfsAvailable) {
    return res.status(503).json({
      success: false,
      error: 'IPFS not available. Benchmarking requires a running IPFS node.',
      demoMode: true
    });
  }
...
wss.on('connection', (ws) => {
  ...
  const interval = setInterval(async () => {
    try {
      if (!ipfsAvailable) {
        ws.send(JSON.stringify({ type: 'peers', data: [], count: 0, demoMode: true }));
        return;
      }
      const peers = await ipfs.swarm.peers();
